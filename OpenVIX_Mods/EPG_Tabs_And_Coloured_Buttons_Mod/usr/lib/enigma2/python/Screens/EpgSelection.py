...
class EPGSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session, service = None, zapFunc = None, eventid = None, bouquetChangeCB=None, serviceChangeCB = None, EPGtype = None, StartBouquet = None, StartRef = None, bouquets = None):
        ...
        if self.type == EPG_TYPE_GRAPH or self.type == EPG_TYPE_INFOBARGRAPH:
            ...
            self['key_green'] = Button(_('Planner'))
    ...
    def RecordTimerQuestion(self, manual=False):
        ...
        for timer in self.session.nav.RecordTimer.timer_list:
            ...
            if not manual:
                menu = [(_("Record"), 'CALLFUNC', self.ChoiceBoxCB, self.doRecordTimer), (_("Record Series"), 'CALLFUNC', self.ChoiceBoxCB, self.addAutoTimerSilent)]
    ...
    def OptionDialog(self):
        ...
        if self.changeText != self.EMPTY:
            cur = self['list'].getCurrent()
            event = cur[0]      
            menu = [(_("IMDb Search"), 'CALLFUNC', self.ChoiceBoxCB, self.openIMDb), (_(self.changeText), 'CALLFUNC', self.ChoiceBoxCB, self.OptionDialogAddTimer), (_("Record Series"), 'CALLFUNC', self.ChoiceBoxCB, self.addAutoTimer), (_("Search"), 'CALLFUNC', self.ChoiceBoxCB, self.openEPGSearch)]
            title = "%s?" % event.getEventName()
